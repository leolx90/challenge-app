-- For daily cadence, allow the app to set period_start (user's local date).
-- Only compute it when not provided so trigger doesn't overwrite.
...
begin
  ...
  if c_cadence = 'day' and new.period_start is not null then
    -- App sent period_start (user local date); keep it.
    return new;
  end if;
